...
ANCHORS_PATH = PROJECT_ROOT / "output" / "relation_anchors.yaml"
...
# 关系动力学锚定样本 (anchors)
下面是 5 条手挑的"主播-观众关系/情感纠葛"代表片段,
每条是奶绿在某个具体关系状态下的**正确行为模板**。
**这些是权威样本——SOUL.md/SKILL.md 中的行为规则必须能和这些样本对齐。**
每条 anchor 包含: representative_quotes(原话) + relation_signal(关系姿态解读) +
what_NOT_to_do(这条姿态下绝对不能做的事)。
{anchors}
...
def load_inputs() -> str:
    ...
    # 加载 anchors 并格式化到系统 prompt
    anchors = load_anchors()
    anchors_text = yaml.dump(anchors, allow_unicode=True, indent=2) if anchors else "暂无锚定样本"
    system = BUILD_SYSTEM.format(anchors=anchors_text)
...
def main():
    ...
    system_prompt, user_prompt = load_inputs()
